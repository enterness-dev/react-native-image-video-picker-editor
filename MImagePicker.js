...
export default class MImagePicker extends React.Component {
	...
	setTransform = (filename) => {
		...
		if (trans) {
			this.transformView.current.updateTransform({
				scale: trans.cropperParams.scale,
				translateX: trans.cropperParams.positionX,
				translateY: trans.cropperParams.positionY,
			});
			return;
		}
		this.transformView.current.updateTransform({
	...
	setCropperParams = (cropperParams) => {
		const param = {
			positionX: cropperParams.translateX,
			positionY: cropperParams.translateY,
			scale: cropperParams.scale,
		};
		...
		const crparamIndex = params.findIndex(
			(t) => t.filename == image.filename
		);
		if (crparamIndex === -1) {
			params.push({ filename: image.filename, cropperParams: param });
			this.setState({ cropperParams: params });
	...
	async componentDidMount() {
		...
		this.setState({
			singleSelect: this.props.profile || this.props.max == 1,
		});
		this.setState({
			badgeColor: this.props.badgeColor ? this.props.badgeColor : "white",
		});
	}
	...
	selectImage = (index) => {
		...
		if (newSelected.indexOf(index) === -1) {
			...
			if (
				...
				Alert.alert(
					"Alert",
					`Please select the video lenght under ${maxVideoLen} sec or update your andriod version to abvoe 18`,
					[
						{
							text: "Ok",
						},
			...
			if (
				(this.props.max === 1 || this.props.profile) &&
			...
			if (
				this.state.photos[newSelected[deleteIndex]].node.image
					.filename == this.state.defaultImage.filename
			) {
				newSelected.splice(deleteIndex, 1);
			} else {
				this.setState({
					defaultImage: this.state.photos[newSelected[deleteIndex]]
						.node.image,
					scrollY: new Animated.Value(
						Platform.OS === "ios" ? -HEADER_MAX_HEIGHT : 0
					),
		...
		this.setTransform(
			this.state.photos[newSelected[inx]].node.image.filename
		);
	};
	...
	next = () => {
		this.props.onNext({
			...
			cropperParams: this.state.cropperParams,
			...
			>
				...
				>
					<ViewTransformer
						...
						onTransformGestureReleased={(e) =>
							this.setCropperParams(e)
						}
				...
				<View style={styles.fill}>
					...
					>
						{this.state.defaultImage &&
							this.renderAsset(this.state.defaultImage)}
					</Animated.View>
				</View>
			</SafeAreaView>
		);
	...
	camera = () => {
		const config = this.props.cameraConfig || {
